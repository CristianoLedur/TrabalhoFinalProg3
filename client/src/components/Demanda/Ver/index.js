...
export default function VerDemanda({ demandaSelecionada, handleButtonValidar, closeModal, fetchDemanda, openModal}) {
    ...
    const [ cidadesString, setCidadesString ] = useState('');
    ...
    useEffect(() => {
        setIsLoading(true);
        setCidades(demandaSelecionada.cidade.map(cidade => cidade.nome));
        setCidadesString(demandaSelecionada.cidade.map(cidade => cidade.nome).join(', '));
    ...
    const handleSubmitData = async (dataForm) => {
        ...
        if(updatedDemanda.tipoDemanda === 'sugerida') {
            var route = 'demanda-sugerida';
        } else {
            var route = 'demanda-solicitada';
        }
        console.log('Dados do formulário solicitada:', updatedDemanda);
        try {
            const response = await fetch(`http://localhost:3001/${route}/${updatedDemanda.id}`, {
                method: 'PUT',
                headers: {
                    'Authorization': `Bearer ${token}`,
                    'Content-Type': 'application/json',
                                ...
                                >
                                    <div className="grid gap-4 mb-4 sm:grid-cols-2">
                                        ...
                                        <div className="sm:col-span-2">
                                            <label htmlFor="comentario" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Comentário</label>
                                            <textarea 
                                                id="comentario"
                                                { ...register('comentario')}
                                                rows="3" 
                                                className="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-primary-500 focus:border-primary-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" 
                                                placeholder="Comente o que deve ser ajustado ou qual o motivo da demanda não ser aprovada..."
                                            >
