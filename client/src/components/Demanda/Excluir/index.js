...
export default function ExcluirAtiviade({ demandaSelecionada, closeModal }) {
    const handleDestroyDemanda = async () => {
        const token = getCookie('Authorization');
        if(demandaSelecionada.tipoDemanda === 'sugerida') {
            var route = 'demanda-sugerida';
        } else {
            var route = 'demanda-solicitada';
        }
        try {
            const response = await fetch(`http://localhost:3001/${route}/${demandaSelecionada.id}`, {
                method: 'DELETE',
                headers: {
    ...
    return (
        <div id="popup-modal" tabIndex="-1" className="fixed top-0 left-0 right-0 z-50 p-4 flex justify-center items-center overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] max-h-full bg-gray-700/50">
            <div className="relative w-full max-w-md max-h-full">
                <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div className="p-6 text-center">
                        ...
                        <h3 className="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Você tem certeza de que deseja excluir esta demanda?</h3>
                        <button 
                            onClick={handleDestroyDemanda}
